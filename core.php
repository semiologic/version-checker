...
class sem_update_core {
		...
		if ( !$done ) {
			if ( strpos($wp_filesystem->method, 'ftp') !== false ) {
				if ( $wp_filesystem->link ) {
					...
					if ( $wp_filesystem->ftp->_timeout < 600 )
						$wp_filesystem->ftp->SetTimeout(600);
					$done = true;
				}
			} else {
				$done = true;
	...
	function ob_start() {
		...
		echo '<p>'
			. __('The single most common reason upgrades fail when using the "FTP filesystem" are server timeouts.', 'version-checker')
			. '</p>' . "\n";
		
		echo '<p>'
			. __('Most of the remaining time, failed upgrades will be related to your server\'s configuration, and should be reported to your host. Before you do, however:', 'version-checker')
			. '</p>' . "\n";
